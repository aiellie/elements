...
- Attachments travel inside the request body (`chat-files.ts`): images shrunk to 1568px and sent as data, PDFs as data, text and code read in as text, and folders file by file without `node_modules`, `.git` or build output. Every request resends the whole chat, and hosts cap a body (Vercel at 4.5 MB), so a chat's attachments share a 4 MB budget, newest first. The route keeps only text and `data:` files of the types every provider reads, so it never fetches an address a request names. Bigger files would need uploading to storage first and sending a link.
