...
const attachmentVariants = cva(
  "group/attachment relative flex w-fit max-w-full min-w-0 shrink-0 flex-wrap rounded-lg border bg-card text-card-foreground transition-colors focus-within:border-ring has-[>a,>button]:hover:bg-muted/50 data-[state=error]:border-destructive/20 data-[state=idle]:border-dashed motion-reduce:transition-none",
  {
    variants: {
      size: {
        default:
...
function AttachmentTitle({
  ...
  return (
    <span
      ...
      className={cn(
        "block max-w-full min-w-0 truncate font-medium group-data-[state=processing]/attachment:shimmer group-data-[state=uploading]/attachment:shimmer motion-reduce:shimmer-none",
        className
      )}
      {...props}
    />
...
function AttachmentGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "no-scrollbar flex min-w-0 scroll-fade-x snap-x snap-mandatory scroll-px-1 gap-2 overflow-x-auto overscroll-x-contain py-1 *:data-[slot=attachment]:flex-none *:data-[slot=attachment]:snap-start",
        className
      )}
      {...props}
    />
